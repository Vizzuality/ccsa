  ...
  () => ({
    async create(ctx) {
      ...
      if (ctx.state.user && ctx.state.user.email) {
        ...
        try {
          strapi.log.info(
            `Sending email confirmation to the author of Collaborator Edit suggestion`
          );
    ...
    async update(ctx) {
      ...
      if (newReviewStatus === "approved" || newReviewStatus === "declined") {
        ...
        if (suggestion && suggestion.author && suggestion.author.email) {
          ...
          try {
            strapi.log.info(
              `Sending email notification to the author about the Collaborator Suggestion review status change`
            );
            ...
            if (newReviewStatus === "approved") {
              ...
              htmlContent = `<h3>Your Collaborator suggestion has been ${newReviewStatus}</h3>
