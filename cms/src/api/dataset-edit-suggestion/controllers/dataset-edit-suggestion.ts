  ...
  () => ({
    async create(ctx) {
      ...
      if (ctx.state.user && ctx.state.user.email) {
        ...
        try {
          strapi.log.info(
            `Sending email confirmation to the author of Dataset Edit suggestion`
          );
        ...
        try {
          ...
          if (adminRole && adminRole.users.length > 0) {
            ...
            for (const admin of adminRole.users) {
              await strapi.plugins["email"].services.email.send({
                to: admin.email,
                subject: "Dataset Edit Suggestion Created",
                html: `<h3>Dataset Suggestion Created</h3>
                     <p>A Dataset suggestion on the Climate Smart Map has been created.  To review the suggestion please access the <a href="${dashboardUrl}/dashboard">Dashboard</a>.  Thank you for your support.</p>`,
              });
            }
    ...
    async update(ctx) {
      ...
      if (newReviewStatus === "approved" || newReviewStatus === "declined") {
        ...
        const suggestion = await strapi
          .query("api::dataset-edit-suggestion.dataset-edit-suggestion")
          .findOne({
        ...
        if (suggestion && suggestion.author && suggestion.author.email) {
          ...
          try {
            strapi.log.info(
              `Sending email notification to the author about the Dataset Suggestion review status change`
            );
            ...
            if (newReviewStatus === "approved") {
              ...
              htmlContent = `<h3>Your Dataset suggestion has been ${newReviewStatus}</h3>
